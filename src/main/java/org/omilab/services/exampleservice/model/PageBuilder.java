...
public class PageBuilder {
    ...
    public String showThread(ForumThread forumThread, Integer i){
       return "<div class=\"media border p-3\">\n" +
               ...
               "  </div>\n" +
               "</div>"
               + "<button type=\"button\" class=\"btn btn-lg btn-outline-success\" onclick=\"javascript:submitform('thread'" + forumThread.getThreadId() +"');\">Post an Answer</button>";
    }
    ...
    public String loggedInNav(ForumUser forumUser){
        return "<!DOCTYPE html>\n" +
                ...
                "</script>\n" +
                "<style>\n" +
                ".list-group, .media{ max-height: 300px;\n" +
                "    margin-bottom: 10px;\n" +
                "    overflow:scroll;\n" +
